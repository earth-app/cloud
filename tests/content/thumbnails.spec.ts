...
function* mohoEntries(): Generator<{ relativePath: string; name: string; lineIndex: number }> {
	for (const [absolute, contents] of Object.entries(ALL_MOHO_CSVS)) {
		...
		for (let i = 0; i < lines.length; i++) {
			// mirror moho's own loader: it strips the BOM and CR, then drops blank and #-comment
			// lines before parsing. re-reading the csvs without that rule fed header comments like
			// "# generated by scripts/fetch-places.ts" to the birthday parser as if they were rows.
